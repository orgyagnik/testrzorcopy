...
import { Card, Form, Dropdown, Spinner, Button, Col, Row, InputGroup, OverlayTrigger, Tooltip, Offcanvas } from 'react-bootstrap';
...
function Contacts({ name, userData }) {
        ...
        APIService.getContactLists(params)
            .then((response) => {
                if (response.data?.status) {
                    ...
                    let activecontactListData = newData.map(({ active, id }) => {
                        if (active === 1)
                            return id;
                        else
                            return 0;
                        ...
                        exportData.push(
                            {
                                ...
                                last_login: item.last_login ? moment(item.last_login).format(display_date_format) : '',
    ...
    let columns = [
        ...
        },
        {
            Header: 'Last Login',
            id: 'last_login',
            accessor: (row) => row?.last_login && moment(new Date(row?.last_login)).format(display_date_format),
        },
        {
            Header: 'Active',
            id: 'active',
