...
export default function TaskList({
    ...
    data?.map((item) => {
      if (userData?.role_code !== databaseRoleCode.clientCode) {
        exportData.push({
          ...
          assign_member:
            Array.isArray(item.assign_member) && item.assign_member.length > 0
              ? item.assign_member.map((user) => user?.name).join(", ")
              : "",
  ...
  const fetchTaskList = () => {
    ...
    // Add action_value=0 filter for TaskList
    params = params + "&action_value=0";
    ...
    if (selected_status.length > 0) {
      // Check if there are selected statuses
    ...
    APIService.getArticleList(params).then((response) => {
      ...
      if (response.data?.status) {
        console.log("Task List Response:", response.data.data); // Debug here
  ...
  const handleReject = (taskId) => {
    // Find the "Rejected" status ID from the taskStatusList
    const rejectedStatus = taskStatusList.find(
      (status) => status.label.toLowerCase() === "rejected"
    );
    ...
    const params = {
      taskid: taskId, // Ensure this matches the expected parameter name in the backend
      status: rejectedStatus.id, // Update the status to "Rejected"
    };

    APIService.updateTaskStatus(params)
      .then((response) => {
        if (response.data?.status) {
          toast.success("Task status updated to Rejected", {
            position: toast.POSITION.TOP_RIGHT,
          });
          setRefreshForNewPage((prev) => !prev); // Refresh the task list
        } else {
        ...
        <div className="p-3">
              ...
              <div className="d-flex align-items-center">
                ...
                {row?.original?.rating && row?.original?.task_status === 5 && (
                  <div className="task-rating ms-4 d-inline-block">
                    <i
                      className={`${row?.original?.rating >= 1
                          ? "icon-star"
                          : "icon-star-line"
                        }`}
                    ></i>
                    <i
                      className={`${row?.original?.rating >= 2
                          ? "icon-star"
                          : "icon-star-line"
                        }`}
                    ></i>
                    <i
                      className={`${row?.original?.rating >= 3
                          ? "icon-star"
                          : "icon-star-line"
                        }`}
                    ></i>
                    <i
                      className={`${row?.original?.rating >= 4
                          ? "icon-star"
                          : "icon-star-line"
                        }`}
                    ></i>
                    <i
                      className={`${row?.original?.rating === 5
                          ? "icon-star"
                          : "icon-star-line"
                        }`}
                    ></i>
          ...
          <div className="row-action mt-1">
            ...
            {check(["article.update"], userData?.role.getPermissions) ||
              check(["article.delete"], userData?.role.getPermissions) ? (
              <span className="font-12 px-1">|</span>
            ) : (
              ""
            )}
              ...
              <>
                {userData?.role_code !== databaseRoleCode.clientCode ||
                  ...
                  <span
                    ...
                    onClick={() => {
                      deleteTask(row?.original?.id, row?.original?.keywords, row?.original?.project_id);
                    }}
                  >
                    Delete
                  </span>
          ...
          {check(["article.update"], userData?.role.getPermissions) ? (
            <Dropdown>
              ...
              <Dropdown.Menu popperConfig={popperConfig}>
                ...
                <Dropdown.Item
                  onClick={() => {
                    handleActionChange(row?.original?.id, 1); // Custom action for "Approved"
                  }}
                >
                  Mark as Approved
                </Dropdown.Item>
                            ...
                            >
                              {user.profile_image !== "" &&
                                ...
                                <ReactImageFallback
                                  ...
                                  alt={user.name}
                                  className={`avatar-img ${user?.id === row?.original?.added_by
                                      ? "border-orange-500"
                                      : ""
                                    } `}
                                />
                        ...
                        <SimpleBar className="dropdown-body">
                          {staffListForFilterOnTask.map((drp, index) => (
                            <Dropdown.Item
                              ...
                              key={index}
                              eventKey={drp.id}
                              className={`${selectedAssignedByOnTask.filter(function (arr) {
  ...
  return (
    <div style={{ '--bs-card-bg': 'transparent', backgroundColor: 'transparent' }}>
      <>
                                  ...
                                  <SimpleBar className="dropdown-body">
                                    {staffListForFilter.map((drp, index) => (
                                      <Dropdown.Item
                                        ...
                                        key={index}
                                        eventKey={drp.id}
                                        className={`${selectedAssignedBy.filter(function (
                                          arr
                                        ) {
                                          return arr.id === drp.id;
                                        }).length > 0
                                    ...
                                    {staffListForFilterForRemove.map(
                                      (drp, index) => (
                                        <Dropdown.Item
                                          ...
                                          key={index}
                                          eventKey={drp.id}
                                          className={`${selectedAssignedByForRemove.filter(
                                            function (arr) {
                                              return arr.id === drp.id;
                                            }
                                          ).length > 0
