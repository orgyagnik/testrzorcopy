...
import ArticleListAccordion from './ArticleListAccordion';
...
function Mytask({ userData, name }) {
  ...
  const updateTaskStatus = (id, status, url = null) => {
    let params = {
      taskid: id,
      status: status,
    };
  
    // Include the URL if the status is "Published"
    if (status === 5 && url) { // Assuming 5 is the ID for "Published"
      params["url"] = url;
    }
    ...
    APIService.updateTaskStatus(params)
      .then((response) => {
        if (response.data?.status) {
          ...
          // Handle calendarToggle and taskStroke logic
          if (calendarToggle === 1) {
            let newTaskStroke = taskStroke;
  ...
  useEffect(() => {
    const status_id = 'pending,approved,rejected'; // Show these statuses for ArticlePage
    APIService.getArticleList(status_id) // Pass the status_id to the API call
      .then((response) => {
        if (response.data?.status) {  
          let taskStatusListNew = response.data?.data;

          // Filter to exclude "Pending Approval" and "Rejected"
          taskStatusListNew = taskStatusListNew.filter(status => 
            status.label !== "Pending Approval" && status.label !== "Hold"
          );
...
useEffect(() => {
  ...
  let params = "";
  if (agency !== '') {
      params = "?search_by_agency=" + agency.id;
  }
...
}, [location.search, refresh, refreshForNewPage, projectReload]);
  ...
  useEffect(() => {
    ...
    handleRefreshPage();
  }, [taskSummary['not-started'], taskSummary['in-progress'], taskSummary['internal-review'], taskSummary['awaiting-feedback'], taskSummary['published']]);
    ...
    APIService.getAllClients()
        .then((response) => {
            if (response.data?.status) {
                ...
                setCustomerList([{ label: 'All', value: 0 }, ...newClientList]);
                setCustomerListForFilter(response.data?.data);
            }
        });      
  }, []);
  ...
  return (
    <>
      <Sidebar />
      <ViewTaskModal fromArticlePage={true} showApproveOption={false} showViewTaskModal={showViewTaskModal} setShowViewTaskModal={setShowViewTaskModal} taskId={taskId} setRefreshForNewPage={setRefreshForNewPage} refreshForNewPage={refreshForNewPage} task_type={task_type} siteAddonURL={siteAddonURL} tasksURL={tasksURL} favouriteURL={favouriteURL} search={search} cstShowEditTaskModal={cstShowEditTaskModal} deleteTask={deleteTask} showRatingModal={showRatingModal} setTaskId={setTaskId} isArticleView={true} showArticle={true}/>
      <EditTaskModal pageType="ArticlePage" showEditTaskModal={showEditTaskModal} setShowEditTaskModal={setShowEditTaskModal} taskId={taskId} setRefreshForNewPage={setRefreshForNewPage} refreshForNewPage={refreshForNewPage} task_type={task_type} siteAddonURL={siteAddonURL} tasksURL={tasksURL} favouriteURL={favouriteURL} search={search} deleteTask={deleteTask} cstShowViewTaskModal={cstShowViewTaskModal} showRatingModal={showRatingModal} setTaskId={setTaskId} />
        ...
        <Dropdown.Item key={index} onClick={() => {/* Handle filter action */}}>
                ...
                {userData.role_code !== databaseRoleCode.agencyCode && userData.role_code !== databaseRoleCode.clientCode && userData.role_code !== databaseRoleCode.agencyMemberCode &&
                  <>
                    <div className='selact-agency-dropdown ms-5 ms-xxl-8 d-flex align-items-center'>
                      ...
                      <Select styles={customStyles} className="control-md custom-select bg-transparent" classNamePrefix="select" options={agencyList1} onChange={handleAgencySelect}
                        value={agencyList1.filter(function (option, index) {
                          if (agency !== '') {
                            return option.value === agency?.id;
                          }
                          else {
                            if (index === 0) {
                              return true;
          ...
          <div className="inner-content">
                    ...
                    <Card className="rounded-12 border border-gray-100 leave-card h-100">
                      <Card.Body className="p-3 px-xxl-4">
                        <Row className="align-items-center">
                          <Col>
                            <span className="h2 mb-0">{taskSummary['not-started'] || 0}</span>
                              <span className="caption text-gray-600 d-block mb-0">Not Started</span>
                            {/* <span className="font-12 caption text-gray-600 d-block mb-1">Tasks assigned to me: {taskSummary['not-started-results-assign-me'] || 0}</span> */}
                      </Col>
                    </Row>
                  </Card.Body>
                </Card>
                  ...
                  {/* <Col xs={6} md={4} xl={4} xxl={2} >
                    <Card className="rounded-12 border border-gray-100 leave-card h-100">
                      <Card.Body className="p-3 px-xxl-4">
                        <Row className="align-items-center">
                          <Col>
                            <span className="h2 mb-0">{taskSummary['pending-approval'] ? taskSummary['pending-approval'] : 0}</span>
                            <span className="text-gray-600 d-block mb-1 font-weight-medium">Pending Approval</span>
                            <span className="font-12 caption text-gray-600 d-block mb-1">Tasks assigned to me: {taskSummary['pending-approval-results-assign-me'] ? taskSummary['pending-approval-results-assign-me'] : 0}</span>
                          </Col>
                        </Row>
                      </Card.Body>
                    </Card>
                      ...
                      <Card.Body className="p-3 px-xxl-4">
                        <Row className="align-items-center">
                          <Col>
                            ...
                            <span className="caption d-block mb-0 text-primary">In Progress</span>
                  ...
                  <Col xs={6} md={4} xl={4} xxl={2} >
                    <Card className="rounded-12 border border-gray-100 leave-card h-100">
                      <Card.Body className="p-3 px-xxl-4">
                        <Row className="align-items-center">
                          <Col>
                            <span className="h2 mb-0">{taskSummary['internal-review'] ? taskSummary['internal-review'] : 0}</span>
                            <span className="caption d-block mb-0 text-warning">Internal Review</span>
                            {/* <span className="font-12 caption text-gray-600 d-block mb-1">Tasks assigned to me: {taskSummary['internal-review-assign-me'] ? taskSummary['internal-review-assign-me'] : 0}</span> */}
                          </Col>
                        </Row>
                      </Card.Body>
                    </Card>
                      ...
                      <Card.Body className="p-3 px-xxl-4">
                        <Row className="align-items-center">
                          <Col>
                            ...
                            <span className="caption d-block mb-0 text-info">Awaiting Feedback</span>
                  ...
                  {/* <Col xs={6} md={4} xl={4} xxl={2} >
                    <Card className="rounded-12 border border-gray-100 leave-card h-100">
                      <Card.Body className="p-3 px-xxl-4">
                        <Row className="align-items-center">
                          <Col>
                            <span className="h2 mb-0">{taskSummary['hold'] ? taskSummary['hold'] : 0}</span>
                            <span className="d-block mb-1 font-weight-medium text-danger">On Hold</span>
                            <span className="font-12 caption text-gray-600 d-block mb-1">Tasks assigned to me: {taskSummary['hold-assign-me'] ? taskSummary['hold-assign-me'] : 0}</span>
                          </Col>
                        </Row>
                      </Card.Body>
                    </Card>
                  ...
                  <Col xs={6} md={4} xl={4} xxl={2} >
                    <Card className="rounded-12 border border-gray-100 leave-card h-100">
                      <Card.Body className="p-3 px-xxl-4">
                        <Row className="align-items-center">
                          <Col>
                            <span className="h2 mb-0">{taskSummary['published'] ? taskSummary['published'] : 0}</span>
                            <span className="caption d-block mb-0 text-success">Published</span>
                            {/* <span className="font-12 caption text-gray-600 d-block mb-1">Tasks assigned to me: {taskSummary['published-assign-me'] ? taskSummary['published-assign-me'] : 0}</span> */}
                          </Col>
                        </Row>
                      </Card.Body>
                    </Card>
            ...
            {calendarToggle && calendarToggle === 3 &&
                          <div className={`card rounded-10 p-6 all-dev-task all-dev-task-with-checkbox`}>
                            {taskStatusList.length > 0 &&
                              <ArticleList userData={userData} agency={agency} assigned_id={assignedToMe === true ? userData?.id : 0} myFollowingTasks={myFollowingTasks === true ? userData?.id : 0} staffId={staffId} recurringTask={recurringTask} project={project} deleteTask={deleteTask} cstShowViewTaskModal={cstShowViewTaskModal} cstShowEditTaskModal={cstShowEditTaskModal} updateTaskStatus={updateTaskStatus} taskStatusList={taskStatusList} refreshForNewPage={refreshForNewPage} setRefreshForNewPage={setRefreshForNewPage} refreshForList={refreshForList} setRefreshButtonProcess={setRefreshButtonProcess} task_type={task_type} showPriorityButton={showPriorityButton} setShowPriorityButton={setShowPriorityButton} customerId={filterCustomerId > 0 ? filterCustomerId : customerId} filterStartDate= {filterStartDate} filterEndDate={filterEndDate} lastOpenedTaskId = {lastOpenedTaskId}/>
                            }
                          </div>
                        }
