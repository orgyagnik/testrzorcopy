...
import { DELETE_CUSTOM_TASKBOARD, DONE_CUSTOM_TASKBOARD, DELETE_AI_DEVELOPER } from '../../modules/lang/Taskboard';
...
function AiTaskboard({ userData, name }) {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            ...
            APIService.getAiTaskBoardList(params)
                .then((response) => {
                    if (response.data?.status) {
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        let params = {};
                        params["id"] = id;
                        APIService.removeAiTaskboard(params)
                            .then((response) => {
                                if (response.data?.status) {
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        ...
                        params["id"] = id;
                        params["status"] = 1;
                        APIService.doneAiTaskboard(params)
                            .then((response) => {
                                if (response.data?.status) {
                                    let newTaskboard = taskBoardData?.taskboard_lists?.filter(function (arr) {
                                        return arr.id !== id;
    ...
    const addTaskboardCard = async () => {
        ...
        if (Object.keys(validate).length) {
            ...
            if (taskBoardId > 0) {
                ...
                APIService.updateAiTaskboardCard(params)
                    .then((response) => {
                        if (response.data?.status) {
                            toast.success(response.data?.message, {
                                position: toast.POSITION.TOP_RIGHT
                ...
                params['happy_status'] = clientHappyStatus;
                params['priority_order'] = priority;
                APIService.addAiTaskboardCard(params)
                    .then((response) => {
                        if (response.data?.status) {
                            toast.success(response.data?.message, {
                                position: toast.POSITION.TOP_RIGHT
    ...
    const onDragEndDrop = (result) => {
        ...
        APIService.setAiStaffListPriority(params)
            .then((response) => {
                if (response.data?.status) {
                    setReloadTaskboard(!reloadTaskboard);
                }
            ...
            <div className="main-content">
                ...
                <div className="inner-content py-0 px-0">
                    <div className='taskboard-page'>
                        {check(['ai_taskboard.create'], userData?.role.getPermissions) &&
                            <>
                                            ...
                                            <div className="card-col col-12" key={emp_index}>
                                                <div className="task-card rounded-10">
                                                    <div className="d-flex align-items-center">
                                                        ...
                                                        <PermissionCheck permissions={['ai_taskboard.create']}>
                                                        ...
                                                        {taskBoardData?.taskboard_lists?.filter(function (arr) { return arr.staff_id === emp.staff_id; }).map((taskboard, index) => (
                                                            <div className="inner-card rounded-10 bg-white p-4" key={`${emp_index}-${index}`}>
                                                                <div className='d-flex'>
                                                                    ...
                                                                    {check(['ai_taskboard.update', 'ai_taskboard.delete'], userData?.role.getPermissions) &&
                                                                        <Dropdown className="ms-auto">
                                                                            ...
                                                                            <Dropdown.Menu align="end" className="dropdown-menu-end p-2" popperConfig={popperConfig}>
                                                                                <PermissionCheck permissions={['ai_taskboard.update']}>
                                                                                    <Dropdown.Item onClick={() => { handleEditTaskBoard(taskboard.id) }}>Edit</Dropdown.Item>
                                                                                </PermissionCheck>
                                                                                <PermissionCheck permissions={['ai_taskboard.delete']}>
...
export default connect(mapStateToProps)(AiTaskboard)